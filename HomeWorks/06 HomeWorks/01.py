...
# Писать нужно с использованием функций

def create_battle_field(fieldsize):
    field = []
    #fieldsize = 3
    ...
    empty_line2 = ["■ "] * fieldsize  # чтобы он стал действительным
...
def print_field(fieldsize): #распечатка поля в консоль с нумерацией строк и столбцов
    print("  ", end="")
...
def game_loop():
    ...
    while x_coordinate_ship != x_coordinate_strike and y_coordinate_ship != y_coordinate_strike:
        x_coordinate_strike = int(
            input(f"Введите координату по ширине поля(строку) от 1 до {fieldsize} включительно: "))  # координаты удара
        y_coordinate_strike = int(
            input(f"Введите координату по высоте поля(столбец) от 1 до {fieldsize} включительно: "))
